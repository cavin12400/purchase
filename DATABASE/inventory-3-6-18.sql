...
INSERT INTO `tbl_purchase_detail` (`product_id`, `supplier_id`, `purchase_id`, `qty_IN`, `total_amount`, `purchase_price`, `selling_price`, `uniqID`) VALUES
(00000002, 00000000, 00000002, 4, 16, 4, 4, 'uniq5a9d23dd06004'),
(00000002, 00000002, 00000002, 75, 375, 5, 6, 'uniq5a9d23e628cbf'),
(00000002, 00000002, 00000003, 1, 234, 234, 4, 'uniq5a9d275e946af'),
(00000001, 00000000, 00000003, 5, 20, 4, 2, 'uniq5a9d277cd48f9'),
(00000002, 00000000, 00000003, 2, 90, 45, 6, 'uniq5a9d2773cf105'),
(00000001, 00000002, 00000003, 5, 30, 6, 7, 'uniq5a9d276ec3c35'),
(00000001, 00000002, 00000004, 32, 96, 3, 21321, 'uniq5a9d43c192227'),
(00000001, 00000002, 00000005, 4, 16, 4, 4, 'uniq5a9d43d6179e3'),
(00000002, 00000000, 00000005, 4, 16, 4, 44, 'uniq5a9d43d07970b'),
(00000002, 00000000, 00000008, 4, 16, 4, 4, 'uniq5a9d445337268'),
(00000002, 00000000, 00000009, 3, 9, 3, 3, 'uniq5a9dcf4aedcc9');
...
(00000002, '2018-03-15', 00000001, '2018-03-05', 00000000, 391, 36, 355, 'edit e'),
(00000003, '2018-03-08', 00000001, '2018-03-05', 00000000, 374, 374, 0, 'madami to pre!'),
(00000004, '2018-03-16', 00000001, '2018-03-05', 00000000, 96, 34, 62, '23'),
(00000005, '2018-03-28', 00000001, '2018-03-05', 00000000, 32, 14, 18, ''),
(00000006, '2018-03-15', 00000001, '2018-03-05', 00000000, 16, 15, 1, 'dummy'),
(00000007, '2018-03-17', 00000001, '2018-03-05', 00000000, 16, 14, 2, '455'),
(00000008, '2018-03-17', 00000001, '2018-03-05', 00000000, 0, 14, 0, '455'),
(00000009, '2018-03-06', 00000001, '2018-03-06', 00000000, 9, 9, 0, 'dsad');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `purchase_searcher`  AS  select `tbl_purchase_order`.`purchase_id` AS `purchase_id`,group_concat(distinct `tbl_supplier`.`supplier_name` separator ', ') AS `supplier_name`,`tbl_purchase_order`.`Date_Ordered` AS `Date_Ordered`,group_concat(distinct `tbl_product`.`product_name` separator ', ') AS `product_name`,`tbl_purchase_order`.`total_amount` AS `total_amount`,`tbl_purchase_order`.`total_payment` AS `total_payment`,`tbl_purchase_order`.`total_balance` AS `total_balance` from (((`tbl_purchase_detail` join `tbl_purchase_order` on((`tbl_purchase_detail`.`purchase_id` = `tbl_purchase_order`.`purchase_id`))) join `tbl_supplier` on((`tbl_purchase_detail`.`supplier_id` = `tbl_supplier`.`supplier_id`))) join `tbl_product` on((`tbl_purchase_detail`.`product_id` = `tbl_product`.`product_id`))) group by `tbl_purchase_order`.`purchase_id` ;
...
--
ALTER TABLE `tbl_purchase_order`
  MODIFY `purchase_id` int(8) UNSIGNED ZEROFILL NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;
--
